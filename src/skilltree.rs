...
impl Builder {
    ...
    /// One branch (port of `branch`): 2 minors, notable, split loop, notable, minor, keystone.
    #[allow(clippy::too_many_arguments)] // it IS the branch definition's arity (mirrors the JS)
    fn branch(
        &mut self,
        key: &str,
        deg: f64,
        minor: Vec<(&'static str, f64)>,
        minor_name: &'static str,
        lane_a: (&'static str, Vec<(&'static str, f64)>),
        lane_b: (&'static str, Vec<(&'static str, f64)>),
        ...
        // Max HP is SPRINKLED: every branch's notables also grant a little (see the JS note).
        ...
        let m1 = n("m1", p(26.0, 0.0), "minor", minor_name, minor.clone(), 1);
        let m2 = n("m2", p(50.0, 0.0), "minor", minor_name, minor.clone(), 1);
        let n1i = n("n1", p(76.0, 0.0), "notable", n1.0, hp_add(&n1.1, 4.0), 2);
        let a1 = n("a1", p(102.0, -22.0), "minor", lane_a.0, lane_a.1.clone(), 2);
        let a2 = n("a2", p(128.0, -22.0), "minor", lane_a.0, lane_a.1.clone(), 3);
        let b1 = n("b1", p(102.0, 22.0), "minor", lane_b.0, lane_b.1.clone(), 2);
        let b2 = n("b2", p(128.0, 22.0), "minor", lane_b.0, lane_b.1.clone(), 3);
        let n2i = n("n2", p(152.0, 0.0), "notable", n2.0, hp_add(&n2.1, 6.0), 3);
        let m3 = n("m3", p(176.0, 0.0), "minor", minor_name, minor, 4);
        let ksi = n("ks", p(202.0, 0.0), "keystone", ks.0, ks.1, 5);
        let start = self.idx("start");
        for (x, y) in [(start, m1), (m1, m2), (m2, n1i), (n1i, a1), (a1, a2), (a2, n2i), (n1i, b1), (b1, b2), (b2, n2i), (n2i, m3), (m3, ksi)] {
            self.link(x, y);
        }
    }
}
...
fn build() -> Vec<Node> {
    ...
    b.branch("war", 0.0, vec![("melee", 0.02)], "+2% MELEE DMG",
        ("+3% MELEE DMG", vec![("melee", 0.03)]),
        ("+1% CRIT", vec![("crit", 0.01)]),
        ("BRUTE FORCE", vec![("melee", 0.05), ("knock", 0.2)]),
        ("WARLORD", vec![("melee", 0.08)]),
        ("JUGGERNAUT", vec![("melee", 0.25), ("move", -0.05)]));
    b.branch("bld", 45.0, vec![("leech", 0.006)], "+0.6% LIFESTEAL",
        ("+0.8% LIFESTEAL", vec![("leech", 0.008)]),
        ("+1 REGEN", vec![("regen", 1.0)]),
        ("OPEN VEINS", vec![("leech", 0.015), ("melee", 0.03)]),
        ("RED FEAST", vec![("leech", 0.02), ("regen", 2.0)]),
        ("VAMPIRE", vec![("leech", 0.06), ("melee", 0.1), ("regen", -3.0)]));
    b.branch("for", 90.0, vec![("luck", 0.03)], "+3% DROP RATE",
        ("+4% DROP RATE", vec![("luck", 0.04)]),
        ("+6% COIN", vec![("coin", 0.06)]),
        ("KEEN EYE", vec![("luck", 0.08), ("magnet", 6.0)]),
        ("GOLDEN TOUCH", vec![("coin", 0.15), ("luck", 0.05)]),
        ("MIDAS", vec![("coin", 0.4), ("luck", 0.2), ("maxhp", -10.0)]));
    b.branch("mag", 135.0, vec![("spell", 0.02)], "+2% SPELL DMG",
        ("+2 MAX MP", vec![("maxmana", 2.0)]),
        ("+1 MANA REGEN", vec![("manaregen", 1.0)]),
        ("ATTUNEMENT", vec![("spell", 0.06), ("maxmana", 3.0)]),
        ("ARCHMAGE", vec![("spell", 0.1), ("manaregen", 1.0)]),
        ("ARCHON", vec![("spell", 0.4), ("maxmana", 8.0), ("maxhp", -15.0)]));
    b.branch("wnd", 180.0, vec![("move", 0.02)], "+2 SPEED",
        ("+3 SPEED", vec![("move", 0.03)]),
        ("+3% HASTE", vec![("haste", 0.03)]),
        ("QUICKSTEP", vec![("move", 0.04), ("haste", 0.02)]),
        ("TEMPEST", vec![("haste", 0.05), ("move", 0.03)]),
        ("GALESOUL", vec![("haste", 0.12), ("move", 0.15), ("maxhp", -20.0)]));
    b.branch("pre", 225.0, vec![("crit", 0.005)], "+0.5% CRIT",
        ("+0.8% CRIT", vec![("crit", 0.008)]),
        ("+5% CRIT DMG", vec![("critmult", 0.05)]),
        ("PRECISION", vec![("crit", 0.015), ("critmult", 0.06)]),
        ("EXECUTIONER", vec![("critmult", 0.2), ("crit", 0.01)]),
        ("ASSASSIN", vec![("crit", 0.1), ("critmult", 0.3), ("melee", -0.1)]));
    b.branch("gth", 270.0, vec![("gather", 0.04)], "+4% HARVEST YIELD",
        ("+5% HARVEST YIELD", vec![("gather", 0.05)]),
        ("+5 PICKUP RANGE", vec![("magnet", 5.0)]),
        ("FORAGER", vec![("gather", 0.1), ("magnet", 4.0)]),
        ("HARVESTER", vec![("gather", 0.12), ("luck", 0.06)]),
        ("BOUNTIFUL", vec![("gather", 0.4), ("luck", 0.15), ("move", -0.04)]));
    b.branch("crf", 315.0, vec![("craft", 0.03)], "+3% MATERIAL SAVE",
        ("+4% MATERIAL SAVE", vec![("craft", 0.04)]),
        ("+5% COIN", vec![("coin", 0.05)]),
        ("TINKERER", vec![("craft", 0.08), ("coin", 0.05)]),
        ("MASTERWORK", vec![("craft", 0.12), ("coin", 0.08)]),
        ("ARTIFICER", vec![("craft", 0.5), ("coin", 0.2), ("melee", -0.05)]));
