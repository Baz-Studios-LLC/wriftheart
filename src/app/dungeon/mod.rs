...
mod ledger;
mod rune;
pub use keys::DungeonKeys;
pub use ledger::{DgSave, DungeonLedger};
pub(crate) use ledger::{apply_ledger, serialize_run};
pub use rune::{RuneActivate, WarpRune};
...
impl Plugin for DungeonPlugin {
    fn build(&self, app: &mut App) {
        app.add_message::<RuneActivate>().add_systems(Update, (dress_castle, victory_tick, pit_anim, descent_overlay)).init_resource::<InDungeon>().init_resource::<DungeonLights>().init_resource::<DungeonLedger>().init_resource::<Relics>().init_resource::<Victory>().init_resource::<PitFalling>().init_resource::<Descending>().init_resource::<DungeonKeys>().add_systems(Update, (breathe, keys::clear_keys_outside, keys::key_hud)).add_systems(
            bevy::app::FixedUpdate,
            (
                enter_dungeon.after(super::interior::door_enter),
                rune::rune_tick.before(navigate),
                navigate,
